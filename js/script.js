window.addEventListener('DOMContentLoaded', () => {
    ...
    function getTimeRemaining(endtime) { // задача функции - получить разницу между датами
        const t = Date.parse(endtime) - Date.parse(new Date()),
            //получим количество милисекунд до количества времени до которого нам нужно будет дойти.
            // в переменную t получим разницу во времени в количестве мс
...
/* Вариант 2
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = deleteNotDigits(width) * (slides.length - 2);
        } else {
            offset -= deleteNotDigits(width);
        }
